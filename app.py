...
BLOCKED_AGENTS = [
    'curl', 'wget', 'httpie', 'python', 'go-http', 'java/',
    'ruby', 'perl', 'php/', 'libwww', 'lwp-',
    'node-fetch', 'undici', 'axios', 'got/', 'superagent',
    'request/', 'aiohttp', 'httpx',
    'bot', 'crawl', 'spider', 'scan', 'slurp', 'archive',
    'selenium', 'headless', 'phantom', 'puppeteer', 'playwright',
    'mechanize',
...
@app.before_request
def filter_traffic():
    ua = request.user_agent.string.lower().strip()
...
def secure_redirect():
    ...
    if not target:
        return render_template(
            ...
            error_message="Error: Missing 'url' parameter"
        ), 400
    ...
    allowed_hosts = {'ads.luarmor.net'}
    if parsed.hostname not in allowed_hosts or '@' in (parsed.netloc or ''):
        return render_template(
            'error.html',
            error_code='403',
            error_title='Forbidden',
            error_message='Unauthorized redirect target'
        ), 403
